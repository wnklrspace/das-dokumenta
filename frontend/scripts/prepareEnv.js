...
async function prepareEnv() {
	...
	const iPAddressSettingRegex = /(REACT_APP_IP_ADDRESS=.*)/;
	const backendApiSettingRegex = /(REACT_APP_BACKEND_URL=.*)/;
	...
	if (fs.existsSync('.env')) {
		...
		if (backendApiSettingTemplateValue === undefined)
			throw new Error(
				'Setting "REACT_APP_BACKEND_URL" not found in .env.example'
			);

		const actualIpAddressSettingRegex = `REACT_APP_IP_ADDRESS=${connectedIP}`;
		const actualBackendApiSettingValue = `REACT_APP_BACKEND_URL=http://${connectedIP}:4000`;
